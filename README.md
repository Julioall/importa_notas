...
Extensão simples para Chrome/Edge que adiciona o botão **Importar** na tela de correção rápida de atividades do Moodle.
...
- Encontra a nota pelo campo `quickgrade_IDDOUSUARIO`, quando a atividade tiver nota.
- Importa CSV, TSV ou XLSX com as colunas `nome`, `nota` e `feedback`.
- Valida cabeçalhos, linhas úteis e notas quando informadas antes de liberar a importação.
...
`/mod/assign/view.php?action=grading`. O botão fica ativo quando a tabela contém campos editáveis de **Comentários de feedback**. O campo de **Nota** é usado quando existir, mas não bloqueia o uso da extensão em atividades sem nota.
...
Em atividades sem campo de nota, a extensão ignora a coluna `nota` e importa o feedback normalmente. Se necessário, também aceita arquivo sem a coluna `nota`:

```csv
nome;feedback
tiao do abacate;Texto do feedback aqui
```
...
9. Revise feedback e, quando houver, nota na tabela.
...
- **Sobrescrever nota existente**: aparece apenas quando a atividade tem campo de nota e vem marcada por padrão, porque o Moodle pode exibir `0,00` mesmo quando você deseja importar uma nova nota.
